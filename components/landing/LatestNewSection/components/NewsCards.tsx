...
export default function NewsCards() {
  return (
    <div className="flex flex-col md:flex-row items-center gap-6 md:gap-28 z-10">
      <div className="flex-1 bg-paper rounded-3xl px-4 pb-4 md:px-8 md:pb-10 w-full max-w-xs md:max-w-sm lg:max-w-lg shadow-lg relative overflow-visible pt-[150px] md:pt-[300px]  ">
        ...
        <span className="text-purple-text hover:text-purple-dark transition-colors duration-200 text-[14px] sm:text-[14px] md:text-[16px] lg:text-[18px] xl:text-[20px] cursor-pointer">
          Read more
        </span>
      </div>
      <div className="flex flex-1 flex-col gap-6 md:gap-8 justify-center">
        {cards.map((card, index) => (
          <div
            key={index}
            className="flex-1 bg-paper rounded-3xl py-4 pb-4 md:py-8 md:pb-10  w-full max-w-xs md:max-w-sm lg:max-w-lg shadow-lg relative overflow-visible pl-[60px] md:pl-[100px] pr-[20px]"
          >
            {card.imageSrc && (
            ...
            <p className="font-semibold text-main-text  text-[14px] md:text-[16px] lg:text-[22px] xl:text-[24px] mb-2 md:mb-4">
              {card.text}
            </p>
            <span className="text-purple-text hover:text-purple-dark transition-colors duration-200 text-[14px]  md:text-[16px] lg:text-[18px] xl:text-[20px] cursor-pointer">
              Read more
            </span>
          </div>
        ))}
